...
5. 【重点】游戏结束后按 R 键重新开始时，必须完整重置所有状态：分数归零、蛇身/方块/球等所有元素回到初始位置、所有定时器和游戏循环先清空再重新启动、画布先清空再重新绘制。
6. 【重点】重新开始时不能出现黑屏、白屏或卡死，必须立即恢复成刚打开时的完整画面。
7. 开局给玩家反应时间，不要一生成就立刻死亡。
8. 深色现代化页面，画布居中，禁止出现滚动条，内容自适应屏幕。
9. 角色、方块、蛇身、子弹等必须高对比鲜艳，禁止深色画在深色背景上。
10. canvas 设置 tabindex="0"，加载和点击时让 canvas 获得焦点。
11. 禁止把 Tab 当作游戏按键。
"""
